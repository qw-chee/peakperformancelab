...
Comment: [Feedback must match the verdict. Be enthusiastic and encouraging only for "Strong and positive" and "Weak or Generic". If "Irrelevant", the comment must highlight the problem directly or express concern. Write in laymen terms, be direct, not fluffy. Write in 30 words or less.]"""
        
        response = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": prompt}], max_tokens=300,temperature=0.7,timeout=30)
